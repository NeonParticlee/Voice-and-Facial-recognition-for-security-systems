...
/****************These are the default inner functions, they get called automatically inside of the lib's other functions that need variability in dealing with different data types, initialize the necessary ones of them for your usage purposes**************/		\
			callbackPrinter##descriptor	defaultPrinter##descriptor = (callbackPrinter##descriptor)NULL;																																																\
			callbackPrinterIterative##descriptor defaultPrinter_It##descriptor = (callbackPrinterIterative##descriptor)NULL;																																													\
			callbackPointerClearer##descriptor	defaultClearer##descriptor = (callbackPointerClearer##descriptor)NULL;	/*used for vectors containing dynamic pointers, like a vector of strings*/																											\
			callbackPointerClearer##descriptor	defaultClearer_L2##descriptor = (callbackPointerClearer##descriptor)NULL;	/*used for inner dynamic double pointers, or essentially used for vectors containing double pointer data*/																		\
																																																																		\
																																																																	\
																																																																		\
...
void vecClearPointers##descriptor(Vector##descriptor* vec)																																															\
																																																																								\
{																																																																								\
	if (vec == NULL || vec->data == NULL || vec->currEnd == 0) { fprintf(stderr, "Vector Data is already NULL or malformed/ vector itself is NULL\n"); return; }																										\
...
{																																																																							\
	vecCompFunc##descriptor comparator = (vecCompFunc##descriptor)NULL;																																																												\
	if (ascending)																																																																			\
	{																																																																						\
	...
	qsort(vec->data, vec->currEnd, sizeof(vec->data[0]), (_CoreCrtNonSecureSearchSortCompareFunction)comparator);																																																						\
}																																																																							\
																																																																							\
...
{																																																																						\
	if (vec->currEnd == 0 || vec->data == NULL || defaultPrinter##descriptor == NULL) { fprintf(stderr, "could not print using print for: Vector%s.\n", #descriptor); return; }																										\
																																																										\
...
{																																																																						\
	...
	temp = (dtype*)realloc(vec->data, sizeof(dtype) * vec->length);																																																								\
...
{																																																																						\
	...
	{																																																																					\
		vec->length /= 2;																																																																\
		vecResizeCustom##descriptor(vec, vec->length, 0, clearer);																																																							\
	}																																																																					\
	dtype* newData = (dtype*)malloc(vec->currEnd * sizeof(dtype));																																																						\
	if (newData) {																																																																		\
		for (size_t i = 0; i < vec->currEnd; ++i)																																																										\
...
{																																																																						\
	if (vecToBeCopied == NULL) {Vector##descriptor nullVec = {0}; return nullVec;}																																																													\
	Vector##descriptor vec = { 0 };																																																														\
